    ...
    while K_noncritical != []:
        ...
        while (queue) and S != K:
            ...
            if len(sigmaplus_noncritical) == 1:
                ...
                # Checking if F_{q, \tau \sigma} is invertible and dim(F_{i}(\tau)) = dim(F_{i}(\sigma)) for all i
                dimensions_compatible = True
                for i in range(filtration_length):
                    if F_vector_spaces[i][tau] != F_vector_spaces[i][sigma]:
                        dimensions_compatible = False
                        break
                if dimensions_compatible and np.linalg.det(F_maps[-1][(tau, sigma)]) != 0:
