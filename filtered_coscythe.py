...
def random_simplicial_complex(n_points, max_dim):
    ...
    return K, K_minus, K_plus, F_vector_spaces, F_maps, connecting_maps
...
def filtered_coscythe(K, F_vector_spaces, F_maps, connecting_map, K_minus=None, K_plus=None):
    ...
    F_vector_spaces is a dictionary, with keys elements of K and values lists, where F_vector_spaces[sigma][i] = dim(F_{i+1}(sigma))
        ...
        for beta in x_minus_modified:
            ...
            for alpha in y_plus_modified:
                ...
                if (alpha,beta) in F_maps[-1]:
                    ...
                    r_alphabeta = np.zeros((F_vector_spaces[beta][-1], F_vector_spaces[alpha][-1]))
    ...
    filtration_length = len(F_vector_spaces[K[0]])
